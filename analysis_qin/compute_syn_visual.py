...
tvb_eec = tvb_synaptic[:, 0, ec_loc[0]:ec_loc[-1]+1, 0]
tvb_iec = tvb_synaptic[:, 1, ec_loc[0]:ec_loc[-1]+1, 0]
...
efd1 = np.loadtxt('evd1_abs_syn.out')
efd1_a = np.loadtxt('evd1_a_abs_syn.out')
efd1_b = np.loadtxt('evd1_b_abs_syn.out')
efd1_s = np.loadtxt('evd1_s_abs_syn.out')
ifd1 = np.loadtxt('ivd1_abs_syn.out')
ifd1_a = np.loadtxt('ivd1_a_abs_syn.out')
ifd1_b = np.loadtxt('ivd1_b_abs_syn.out')
ifd1_s = np.loadtxt('ivd1_s_abs_syn.out')
efd2 = np.loadtxt('evd2_abs_syn.out')
efd2_a = np.loadtxt('evd2_a_abs_syn.out')
efd2_b = np.loadtxt('evd2_b_abs_syn.out')
efd2_s = np.loadtxt('evd2_s_abs_syn.out')
ifd2 = np.loadtxt('ivd2_abs_syn.out')
ifd2_a = np.loadtxt('ivd2_a_abs_syn.out')
ifd2_b = np.loadtxt('ivd2_b_abs_syn.out')
ifd2_s = np.loadtxt('ivd2_s_abs_syn.out')
exfs = np.loadtxt('evfs_abs_syn.out')
exfs_a = np.loadtxt('evfs_a_abs_syn.out')
exfs_b = np.loadtxt('evfs_b_abs_syn.out')
exfs_s = np.loadtxt('evfs_s_abs_syn.out')
infs = np.loadtxt('ivfs_abs_syn.out')
infs_a = np.loadtxt('ivfs_a_abs_syn.out')
infs_b = np.loadtxt('ivfs_b_abs_syn.out')
infs_s = np.loadtxt('ivfs_s_abs_syn.out')
exfr = np.loadtxt('evfr_abs_syn.out')
exfr_a = np.loadtxt('evfr_a_abs_syn.out')
exfr_b = np.loadtxt('evfr_b_abs_syn.out')
exfr_s = np.loadtxt('evfr_s_abs_syn.out')
infr = np.loadtxt('ivfr_abs_syn.out')
infr_a = np.loadtxt('ivfr_a_abs_syn.out')
infr_b = np.loadtxt('ivfr_b_abs_syn.out')
infr_s = np.loadtxt('ivfr_s_abs_syn.out')
evga = np.loadtxt('evga_abs_syn.out')
evga_a = np.loadtxt('evga_a_abs_syn.out')
evga_b = np.loadtxt('evga_b_abs_syn.out')
evga_s = np.loadtxt('evga_s_abs_syn.out')
ivga = np.loadtxt('ivga_abs_syn.out')
ivga_a = np.loadtxt('ivga_a_abs_syn.out')
ivga_b = np.loadtxt('ivga_b_abs_syn.out')
ivga_s = np.loadtxt('ivga_s_abs_syn.out')
...
d1_syn = np.sum(efd1 + ifd1 + efd1_a + ifd1_a + efd1_b + ifd1_b + efd1_s + ifd1_s, axis = 1) + np.sum(tvb_ed1+tvb_id1, axis=1)
d2_syn = np.sum(efd2 + ifd2 + efd2_a + ifd2_a + efd2_b + ifd2_b + efd2_s + ifd2_s, axis = 1) + np.sum(tvb_ed2+tvb_id2, axis=1)
fs_syn = np.sum(exfs + infs + exfs_a + infs_a + exfs_b + infs_b + exfs_s + infs_s, axis = 1) + np.sum(tvb_efs+tvb_ifs, axis=1)
fr_syn = np.sum(exfr + infr + exfr_a + infr_a + exfr_b + infr_b + exfr_s + infr_s, axis = 1) + np.sum(tvb_efr+tvb_ifr, axis=1)
ec_syn = np.sum(evga + ivga + evga_a + ivga_a + evga_b + ivga_b + evga_s + ivga_s, axis = 1) + np.sum(tvb_eec+tvb_iec, axis=1)
pfc_syn = np.sum(efd1 + ifd1 + efd1_a + ifd1_a + efd1_b + ifd1_b + efd1_s + ifd1_s + efd2 + ifd2 + efd2_a + ifd2_a + efd2_b + ifd2_b + efd2_s + ifd2_s + exfs + infs + exfs_a + infs_a + exfs_b + infs_b + exfs_s + infs_s +
exfr + infr + exfr_a + infr_a + exfr_b + infr_b + exfr_s + infr_s, axis=1) + np.sum(tvb_ed1+tvb_id1+tvb_ed2+tvb_id2+tvb_efs+tvb_ifs+tvb_efr+tvb_ifr, axis=1)
...
synaptic = np.array([v1_syn, v4_syn, it_syn, fs_syn, d1_syn, d2_syn, fr_syn, lit_syn, ec_syn, pfc_syn])
...
plt.figure()
plt.suptitle('SIMULATED SYNAPTIC ACTIVITY IN EC')
plt.plot(ec_syn)

plt.figure()
plt.suptitle('SIMULATED SYNAPTIC ACTIVITY IN PFC')
plt.plot(pfc_syn)
